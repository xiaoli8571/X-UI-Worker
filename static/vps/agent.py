...
HEADERS = {'Content-Type': 'application/json', 'Authorization': TOKEN, 'User-Agent': 'X-UI-Unified-Agent/2.0'}
...
# 住宅IP代理后端：默认与 X-UI 同域；独立部署 Free-Residential-IP-Proxy-Controller 时，
...
def _ensure_wgcf():
    ...
    request = urllib.request.Request(url, headers={"User-Agent": "X-UI-WARP/1.0"})
