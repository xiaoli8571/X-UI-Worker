...
async function notifyRealtimePublicPolicy(env, db, enabled, pagesOrigin = '') {
    ...
    await fetch(`${configured.replace(/\/$/, '')}/public-policy`, {
        method: 'POST',
        headers: { Authorization: authorization, 'Content-Type': 'application/json', 'X-UI-Pages-Origin': pagesOrigin },
...
async function notifyRealtimeVps(env, db, ip, pagesOrigin = '') {
    ...
    await fetch(`${configured.replace(/\/$/, '')}/notify`, { method: 'POST', headers: { Authorization: authorization, 'Content-Type': 'application/json', 'X-UI-Pages-Origin': pagesOrigin }, body: JSON.stringify({ ip }) });
}
...
async function initializeDbSchema(db) {
    ...
    try { await db.prepare("SELECT sort_order FROM servers LIMIT 1").first(); } catch (e) { try { await db.prepare("ALTER TABLE servers ADD COLUMN sort_order INTEGER DEFAULT 0").run(); } catch(e){} }
    try { await db.prepare("SELECT sort_order FROM nodes LIMIT 1").first(); } catch (e) { try { await db.prepare("ALTER TABLE nodes ADD COLUMN sort_order INTEGER DEFAULT 0").run(); } catch(e){} }
    try { await db.prepare("SELECT name FROM nodes LIMIT 1").first(); } catch (e) { try { await db.prepare("ALTER TABLE nodes ADD COLUMN name TEXT DEFAULT ''").run(); } catch(e){} }
    try { await db.prepare("SELECT address FROM nodes LIMIT 1").first(); } catch (e) { try { await db.prepare("ALTER TABLE nodes ADD COLUMN address TEXT DEFAULT ''").run(); } catch(e){} }
...
export async function onRequest(context) {
    ...
    if (action === "agent_update" && method === "GET") {
        ...
        const assets = { agent: '/vps/agent.py', 'realtime-client': '/vps/realtime_client.py', 'proxy-manager': '/vps/lite_manager.py', 'proxy-server': '/vps/proxy_server.py', 'proxy-installer': '/vps/residential-proxy.sh', 'full-installer': '/vps/xui.sh', 'probe-installer': '/vps/probe.sh', 'probe-agent': '/vps/probe.py' };
    ...
    if (action === "report" && method === "POST") {
     try {
        ...
        if (!xuiServer) {
            return Response.json({ error: "Server has been removed from x-UI panel." }, { status: 403 });
        }
    ...
    if (action === "config" && method === "GET") {
        ...
        if (action === "egress_result" && method === "POST") {
            await ensureDbSchema(db);
            const body = await request.json();
            const ip = body.ip || request.headers.get('X-UI-IP');
    ...
    if (action === "sub" && method === "GET") {
        ...
        for (let node of results) {
            ...
            let link = "";
            let cProxy = "";
            const nodeIp = formatIpForLink(node.address || node.vps_ip);
    ...
    try {
        if (action === "data") {
            const servers = isAdmin
                ...
                : (await db.prepare("SELECT ip, name, cpu, mem, last_report, disk, load, uptime, net_in_speed, net_out_speed, tcp_conn, udp_conn FROM servers ORDER BY sort_order, name").all()).results;
            if (isAdmin) {
            ...
            const nodes = isAdmin ? (await db.prepare("SELECT * FROM nodes ORDER BY sort_order, port").all()).results : (await db.prepare("SELECT DISTINCT n.* FROM nodes n WHERE n.username = ? OR EXISTS (SELECT 1 FROM user_group_members gm JOIN user_group_resources gr ON gr.group_id = gm.group_id WHERE gm.username = ? AND ((gr.resource_type = 'node' AND gr.resource_id = n.id) OR (gr.resource_type = 'vps' AND gr.resource_id = n.vps_ip))) ORDER BY n.sort_order, n.port").bind(currentUser, currentUser).all()).results;
        ...
        if (action === "nodes" && isAdmin) {
            if (method === "GET" && params.path[1] === "export") {
                const { results } = await db.prepare("SELECT id, uuid, vps_ip, protocol, port, sni, private_key, public_key, short_id, relay_type, target_ip, target_port, target_id, enable, traffic_used, traffic_limit, expire_time, username, network, sort_order, name, address FROM nodes ORDER BY vps_ip, sort_order, port").all();
                const stamp = new Date().toISOString().slice(0, 10);
                return Response.json({ nodes: results || [] }, { headers: { "Content-Disposition": `attachment; filename="xui-nodes-${stamp}.json"`, "Cache-Control": "no-store" } });
            }
            if (method === "POST" && params.path[1] === "import") {
                let list;
                try {
                ...
                if (Array.isArray(list) && list[0] && list[0].nodes) list = list[0].nodes;
                if (!Array.isArray(list)) {
                    if (list && Array.isArray(list.nodes)) list = list.nodes;
                    else return Response.json({ error: '导入内容需为 {nodes: [...]} 或节点数组' }, { status: 400 });
                }
                if (list.length > 500) return Response.json({ error: '单次最多导入 500 个节点' }, { status: 400 });
                const protocols = ['VLESS','XTLS-Reality','Reality','Hysteria2','TUIC','Trojan','H2-Reality','gRPC-Reality','AnyTLS','Naive','Socks5','VLESS-Argo','dokodemo-door'];
                const existingVps = (await db.prepare("SELECT ip FROM servers").all()).results.map(r => r.ip);
                const vpsSet = new Set(existingVps);
                const existingIds = new Set((await db.prepare("SELECT id FROM nodes").all()).results.map(r => r.id));
                const statements = [];
                let imported = 0;
                for (const item of list) {
                    const id = String(item && item.id || '').trim();
                    const vpsIp = String(item && item.vps_ip || '').trim();
                    if (!/^[A-Za-z0-9_-]{1,64}$/.test(id) || !vpsSet.has(vpsIp) || existingIds.has(id)) continue;
                    if (!protocols.includes(item.protocol) || !Number.isInteger(Number(item.port)) || Number(item.port) < 1 || Number(item.port) > 65535) continue;
                    if (item.protocol === 'dokodemo-door' && item.relay_type === 'external' && (!String(item.target_ip || '').trim() || !Number.isInteger(Number(item.target_port)) || Number(item.target_port) < 1 || Number(item.target_port) > 65535)) continue;
                    const address = String(item.address || '').trim();
                    if (address && !/^[0-9A-Fa-f:.a-zA-Z-]{2,255}$/.test(address)) continue;
                    existingIds.add(id);
                    statements.push(db.prepare(`INSERT INTO nodes (id, uuid, vps_ip, protocol, port, sni, private_key, public_key, short_id, relay_type, target_ip, target_port, target_id, enable, traffic_used, traffic_limit, expire_time, username, network, sort_order, name, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(id, item.uuid || crypto.randomUUID(), vpsIp, item.protocol, Number(item.port), item.sni || null, item.private_key || null, item.public_key || null, item.short_id || null, item.relay_type || null, item.target_ip || null, item.target_port ? Number(item.target_port) : null, item.target_id || null, item.enable === 0 ? 0 : 1, Number(item.traffic_used) || 0, Number(item.traffic_limit) || 0, Number(item.expire_time) || 0, String(item.username || 'admin').slice(0, 64), item.network || 'tcp', Number(item.sort_order) || 0, String(item.name || '').slice(0, 100), address));
                    imported++;
                }
                if (statements.length) await chunkBatch(db, statements);
                return Response.json({ success: true, imported, total: list.length, skipped: list.length - imported });
            }
            if (method === "POST") { const n = await request.json(); const protocols = ['VLESS','XTLS-Reality','Reality','Hysteria2','TUIC','Trojan','H2-Reality','gRPC-Reality','AnyTLS','Naive','Socks5','VLESS-Argo','dokodemo-door']; if (!/^[A-Za-z0-9_-]{1,64}$/.test(String(n.id || ''))) return Response.json({ error: 'Invalid node id' }, { status: 400 }); if (!protocols.includes(n.protocol)) return Response.json({ error: 'Invalid protocol' }, { status: 400 }); if (!Number.isInteger(Number(n.port)) || Number(n.port) < 1 || Number(n.port) > 65535) return Response.json({ error: 'Invalid port' }, { status: 400 }); if (!(await db.prepare('SELECT ip FROM servers WHERE ip = ?').bind(n.vps_ip).first())) return Response.json({ error: 'VPS not found' }, { status: 404 }); if (n.protocol === 'dokodemo-door') { if (!['internal','external'].includes(n.relay_type)) return Response.json({error:'Invalid relay type'},{status:400}); if (n.relay_type === 'external' && (!String(n.target_ip||'').trim() || !Number.isInteger(Number(n.target_port)) || Number(n.target_port)<1 || Number(n.target_port)>65535)) return Response.json({error:'Invalid relay target'},{status:400}); if (n.relay_type === 'internal' && !(await db.prepare('SELECT id FROM nodes WHERE id = ? AND vps_ip = ?').bind(n.target_id,n.vps_ip).first())) return Response.json({error:'Internal relay target not found on VPS'},{status:400}); } if (await db.prepare("SELECT id FROM nodes WHERE id = ?").bind(n.id).first()) return Response.json({ error: "Node already exists" }, { status: 409 }); let nodeUser = n.username || currentUser; if (nodeUser === 'admin') nodeUser = currentUser; await db.prepare(`INSERT INTO nodes (id, uuid, vps_ip, protocol, port, sni, private_key, public_key, short_id, relay_type, target_ip, target_port, target_id, enable, traffic_used, traffic_limit, expire_time, username, network, name, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(n.id, n.uuid, n.vps_ip, n.protocol, Number(n.port), n.sni||null, n.private_key||null, n.public_key||null, n.short_id||null, n.relay_type||null, n.target_ip||null, n.target_port||null, n.target_id||null, 1, 0, Math.max(0, Number(n.traffic_limit)||0), Math.max(0, Number(n.expire_time)||0), nodeUser, n.network||'tcp', String(n.name || '').slice(0, 100), String(n.address || '').slice(0, 255)).run(); context.waitUntil(notifyRealtimeVps(env, db, n.vps_ip).catch(()=>{})); return Response.json({ success: true }); }
            if (method === "PUT") { const { id, enable, reset_traffic, reorder, vps_ip, ordered_ids, position, name, address } = await request.json(); if (reorder) {
                if (ordered_ids && Array.isArray(ordered_ids)) {
                    if (!vps_ip || ordered_ids.length > 500) return Response.json({ error: 'Invalid reorder request' }, { status: 400 });
                    await db.batch(ordered_ids.map((nodeId, index) => db.prepare("UPDATE nodes SET sort_order = ? WHERE id = ? AND vps_ip = ?").bind(index, nodeId, vps_ip)));
                    return Response.json({ success: true });
                }
                if (position && id) {
                    ...
                    const list = (await db.prepare("SELECT id FROM nodes WHERE vps_ip = ? ORDER BY sort_order, port").bind(node.vps_ip).all()).results.map(r => r.id);
                    const pos = Number(position);
                    if (!Number.isInteger(pos) || pos < 1 || pos > list.length) return Response.json({ error: 'Invalid position' }, { status: 400 });
                    const idx = list.indexOf(String(id));
                    if (idx < 0) return Response.json({ error: 'Node not found' }, { status: 404 });
                    list.splice(idx, 1);
                    list.splice(pos - 1, 0, String(id));
                    await db.batch(list.map((nodeId, index) => db.prepare("UPDATE nodes SET sort_order = ? WHERE id = ?").bind(index, nodeId)));
                    return Response.json({ success: true });
                }
                return Response.json({ error: 'Invalid reorder request' }, { status: 400 });
            } const node = await db.prepare('SELECT vps_ip FROM nodes WHERE id = ?').bind(id).first(); if (!node) return Response.json({ error: 'Node not found' }, { status: 404 }); const statements = []; if (reset_traffic) statements.push(db.prepare("UPDATE nodes SET traffic_used = 0 WHERE id = ?").bind(id)); if (enable !== undefined) statements.push(db.prepare("UPDATE nodes SET enable = ? WHERE id = ?").bind(enable ? 1 : 0, id)); if (name !== undefined) statements.push(db.prepare("UPDATE nodes SET name = ? WHERE id = ?").bind(String(name || '').slice(0, 100), id)); if (address !== undefined) { const addr = String(address || '').trim(); if (addr && !/^[0-9A-Fa-f:.a-zA-Z-]{2,255}$/.test(addr)) return Response.json({ error: 'Invalid node address' }, { status: 400 }); statements.push(db.prepare("UPDATE nodes SET address = ? WHERE id = ?").bind(addr.slice(0, 255), id)); } if (statements.length) await db.batch(statements); context.waitUntil(notifyRealtimeVps(env, db, node.vps_ip).catch(()=>{})); return Response.json({ success: true }); }
     ...
     if (action === "thirdparty" && isAdmin) {
            ...
            if (method === "POST") {
                ...
                if (!url) return Response.json({ error: "请填写订阅链接或节点链接" }, { status: 400 });
                const trimmedUrl = String(url).trim();
                const singleProtocols = ['vless://', 'vmess://', 'trojan://', 'ss://', 'ssr://', 'hysteria2://', 'hy2://', 'hysteria://', 'tuic://', 'naive+https://', 'naive://', 'socks://', 'http://', 'https://'];
                const isSingle = singleProtocols.some(p => trimmedUrl.toLowerCase().startsWith(p));
                const id = crypto.randomUUID();
                const now = Date.now();
                let parsedCount = 0; let parseDebug = {};
                if (isSingle) {
                    try {
                        const result = await parseThirdPartySubscription(trimmedUrl);
                        const { nodes, protocolCounts, debug } = result;
                        parseDebug = { protocolCounts, debug };
                        if (nodes.length === 0) return Response.json({ error: "无法解析该节点链接，请检查格式是否支持" }, { status: 400 });
                        await db.prepare("INSERT INTO third_party_subscriptions (id, name, url, added_at, last_fetched_at) VALUES (?, ?, ?, ?, ?)").bind(id, String(name || '单节点').slice(0, 100), trimmedUrl.slice(0, 500), now, now).run();
                        const stmts = nodes.map(n => db.prepare("INSERT INTO third_party_nodes (id, subscription_id, name, protocol, address, port, uuid, password, sni, public_key, short_id, flow, network, host, path, extra, enable, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(n.id, id, n.name || n.address || '节点', n.protocol, n.address, n.port, n.uuid || '', n.password || '', n.sni || '', n.public_key || '', n.short_id || '', n.flow || '', n.network || '', n.host || '', n.path || '', n.extra || '', 1, now));
                        await chunkBatch(db, stmts);
                        parsedCount = nodes.length;
                    } catch (e) { console.error("导入单节点失败:", e); return Response.json({ error: (e && e.message) || '节点导入失败' }, { status: 400 }); }
                    return Response.json({ success: true, id, parsedCount, debug: parseDebug });
                }
                let subscriptionUrl;
                try { subscriptionUrl = validateSubscriptionUrl(trimmedUrl); } catch (error) { return Response.json({ error: error.message || "订阅链接格式无效" }, { status: 400 }); }
                try {
