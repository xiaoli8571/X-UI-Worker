...
function requestPagesOrigin(request, env) {
  const candidates = [request.headers.get("X-UI-Pages-Origin"), request.headers.get("Origin")].filter(Boolean);
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/agent/ws") {
      ...
      return stub.fetch(doRequest("/ws", request, { "X-UI-IP": ip, "X-UI-ROLE": role }));
    }
    ...
    if (url.pathname === "/dashboard/ticket" && request.method === "POST") {
      ...
      const response = await hub.fetch(new Request("https://hub.internal/ticket", { method: "POST", headers: { "X-UI-USER": "admin" } }));
    ...
    if (url.pathname === "/public/ws") {
      ...
      const hub = env.DASHBOARD_HUB.get(env.DASHBOARD_HUB.idFromName("main"));
      return hub.fetch(doRequest("/public-ws", request, { "X-UI-CLIENT-IP": request.headers.get("CF-Connecting-IP") || "unknown" }));
    }
    ...
    if (url.pathname === "/public-policy" && request.method === "POST") {
      ...
      const body = await request.json().catch(() => ({}));
      const hub = env.DASHBOARD_HUB.get(env.DASHBOARD_HUB.idFromName("main"));
      const response = await hub.fetch(new Request("https://hub.internal/public-policy", { method: "POST", headers: { "X-UI-Public": body.public === true ? "1" : "0" } }));
      return new Response(response.body, { status: response.status, headers: { ...Object.fromEntries(response.headers), ...cors(request, env) } });
    }
...
export class VpsPresence extends DurableObject {
  ...
  async fetch(request) {
    const url = new URL(request.url);
    if (url.pathname === "/ws") {
      const ip = request.headers.get("X-UI-IP") || "";
      const role = request.headers.get("X-UI-ROLE") || "";
    ...
    if (url.pathname === "/dashboard-active" && request.method === "POST") {
      this.setDashboardActivity(request.headers.get("X-UI-Active") === "1", Number(request.headers.get("X-UI-Interval")) || IDLE_STATUS_INTERVAL, Number(request.headers.get("X-UI-Until")) || Date.now() + 300000);
      return json({ success: true });
    }
  ...
  async broadcast() {
    ...
    await hub.fetch(new Request("https://hub.internal/update", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-UI-Presence": "1" },
...
export class DashboardHub extends DurableObject {
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/ticket" && request.method === "POST") {
      ...
      await this.ctx.storage.put(`ticket:${ticket}`, { user: request.headers.get("X-UI-USER") || "", expires: Date.now() + 60000 });
    ...
    if (url.pathname === "/public-ws") {
      const clientIp = request.headers.get("X-UI-CLIENT-IP") || "unknown";
    ...
    if (url.pathname === "/public-policy" && request.method === "POST") {
      const enabled = request.headers.get("X-UI-Public") === "1";
      if (!enabled) {
    ...
    if (url.pathname === "/update" && request.method === "POST") {
      if (request.headers.get("X-UI-Presence") !== "1") return json({ error: "Forbidden" }, 403);
  ...
  async setDashboardActivity() {
    ...
    await Promise.all(servers.slice(0, 100).map(async ({ ip }) => {
      ...
      if (!name) return;
      const presence = this.env.VPS_PRESENCE.get(this.env.VPS_PRESENCE.idFromName(name));
      return presence.fetch(new Request("https://presence.internal/dashboard-active", { method: "POST", headers: { "X-UI-Active": active ? "1" : "0", "X-UI-Interval": String(interval), "X-UI-Until": String(until) } }));
    }));
  }
